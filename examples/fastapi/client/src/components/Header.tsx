  ...
  const handleAction = async (method: string, options: any = {}) => {
    ...
    try {
      ...
      if (onContentChange && response.output) {
        onContentChange(method === "deduplicator" ? response.output : textarea.value + "\n\n" + response.output);
      }
    } catch (error) {
    ...
    try {
      ...
      if (useStream) {
        ...
        await nlpService.generateMarkovStream(
          ...
          (chunk, is_final) => {
            accumulated += chunk;
            if (onContentChange) onContentChange(baseText + accumulated);
        ...
        if (onContentChange && res.output) onContentChange(baseText + res.output);
        setIsGenerating(false);
      }
    } catch (error) {
